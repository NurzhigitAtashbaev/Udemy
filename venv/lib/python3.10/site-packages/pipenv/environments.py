...
class Setting:
    ...
    def __init__(self) -> None:
        ...
        """If set, Pipenv does not load the ``..env.py`` file.

        Default is to load ``..env.py`` for ``run`` and ``shell`` commands.
        """
        ...
        """If set, Pipenv loads the ``..env.py`` file at the specified location.

        Default is to load ``..env.py`` from the project root, if found.
        """
