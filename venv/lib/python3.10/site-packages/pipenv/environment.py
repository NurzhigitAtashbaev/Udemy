...
class Environment:
    ...
    def safe_import(self, name: str) -> ModuleType:
        """Helper utility for reimporting previously imported modules while inside the .env.py"""
        module = None
    ...
    def base_paths(self) -> Dict[str, str]:
        ...
        >>> .env.py = Environment(prefix=project.virtualenv_location, is_venv=True, sources=project.sources)
        >>> import pprint
        >>> pprint.pprint(.env.py.base_paths)
    ...
    def uninstall(self, pkgname, *args, **kwargs):
        ...
        >>> .env.py = Environment("/path/to/.env.py/root")
        >>> with .env.py.uninstall("pytz", auto_confirm=True, verbose=False) as uninstaller:
