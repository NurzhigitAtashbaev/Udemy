...
def load_dot_env(project, as_dict=False, quiet=False):
    """Loads ..env.py file into sys.environ."""
    if not project.s.PIPENV_DONT_LOAD_ENV:
        # If the project doesn't exist yet, check current directory for a ..env.py file
        project_directory = project.project_directory or "."
        dotenv_file = project.s.PIPENV_DOTENV_LOCATION or os.sep.join(
            [project_directory, "..env.py"]
        )
        ...
        if as_dict:
            ...
            if not quiet:
                click.echo(
                    click.style(
                        fix_utf8("Loading ..env.py environment variables..."), bold=True
                    ),
                    err=True,
                )
