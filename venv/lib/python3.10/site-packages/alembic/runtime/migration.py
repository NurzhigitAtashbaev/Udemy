...
class MigrationContext:
    ...
    When inside the ``.env.py.py`` script, the :class:`.MigrationContext`
    is available via the
    :meth:`.EnvironmentContext.get_context` method,
    which is available at ``alembic.context``::

        # from within .env.py.py script
        from alembic import context
        migration_context = context.get_context()

    For usage outside of an ``.env.py.py`` script, such as for
    ...
    database using :meth:`.MigrationContext.get_current_revision`::
        ...
        # in any application, outside of an .env.py.py script
    ...
    def begin_transaction(
        ...
        This method is used within an ``.env.py.py`` script to demarcate where
    ...
    def run_migrations(self, **kw: Any) -> None:
        ...
        is called, typically from within the ``.env.py.py`` script of the
