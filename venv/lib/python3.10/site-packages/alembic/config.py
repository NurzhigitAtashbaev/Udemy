...
class Config:
    ...
    Within an ``.env.py.py`` script, this is available
    ...
    .. warning::
       ...
       ``.env.py.py`` file in use is compatible with the target configuration;
    ...
    Within an ``.env.py.py`` script this can be accessed via the
    ...
    def attributes(self):
        ...
        Use this to pass objects into an .env.py.py script, such as passing
...
class CommandLine:
    ...
    def _generate_args(self, prog: Optional[str]) -> None:
        ...
        parser.add_argument(
            ...
            "custom .env.py.py scripts, e.g. -x "
