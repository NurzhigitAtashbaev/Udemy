...
class TestCase(TestCaseMetaClass(
    ...
    def wait_for_pending_cleanups(self,
        ...
        # Counting the Account greenlets, which we can easily do deterministically,
        # also helps.

        # Always sleep at least once to let other threads run
        sleep(sleep_time)
        quit_after = time() + self.cleanup_max_sleep_seconds
        # TODO: We could add an API that calls us back when a particular Account greenlet is deleted?
    ...
    def setUp(self):
        # Ensure the Account greenlet exists, otherwise the first test
